...
func TestClienCreateSFlow(t *testing.T) {
	...
	c := testClient([]OptionFunc{Timeout(1)}, func(cmd string, args ...string) ([]byte, error) {
		...
		wantArgs := []string{
			"--timeout=1",
			"--",
			"--id=@sflow", "create", "sflow", fmt.Sprintf("agent=%s", agent_ip), fmt.Sprintf("target=\"%s:%s\"", collector_ip, collector_port),
			fmt.Sprintf("header=%s",header_bytes), fmt.Sprintf("sampling=%s",sampling_n), fmt.Sprintf("polling=%s",polling_secs),
			"--",
	...
	sflowID, err := c.VSwitch.CreateSFlow(bridge, agent_ip, collector_ip, collector_port, header_bytes, sampling_n, polling_secs)
	if err != nil {
